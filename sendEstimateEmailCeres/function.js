...
module.exports = async function (requestBody) {
  ...
  await new AWS.SES(SESConfig)
    ...
    .then((res) => {
        ...
        response.travellerStatus = res.statusCode
    })
    .catch((err) => {
        console.log('err: ', err)
        response.traveller = err;
        response.travellerStatus = err.statusCode
        response.travellerError = err.message
    });
  ...
  await new AWS.SES(SESConfig)
    ...
    .then((res) => {
      ...
      response.approverStatus = res.statusCode
    })
    .catch((err) => {
      console.log('err: ', err)
      response.approver = err;
      response.approverStatus = err.statusCode
      response.approverError = err.message
    });

  if (response.approverStatus === 200 && response.travellerStatus === 200) {
    response.statusCode = 200;
  } else if (response.approverStatus === 200 || response.travellerStatus === 200) {
